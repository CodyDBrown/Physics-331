def euler_1d(y0, t0, tf, dt, deriv_func):
    """
    Numerically solved the first order ODE deriv_func using Eulers method
    ...
    t: Array of times the function is evaluated at. You don't need this but it's nice to have to make plots

    """

    t = np.arange(t0, tf, dt) #starts at t0 and makes steps of size dt. Does not get to tf. It gets as
                            #Close as it can to tf, but will always stop short of it.
    n_steps = len(t)
    y = np.zeros(n_steps)  #I want to make an array of zeros that I'll then put y-values into
